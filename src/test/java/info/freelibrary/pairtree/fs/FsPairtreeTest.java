...
import static info.freelibrary.pairtree.MessageCodes.PT_DEBUG_010;
import static info.freelibrary.pairtree.MessageCodes.PT_DEBUG_011;
import static info.freelibrary.pairtree.MessageCodes.PT_DEBUG_012;
import static info.freelibrary.pairtree.MessageCodes.PT_DEBUG_013;
import static info.freelibrary.pairtree.MessageCodes.PT_DEBUG_014;
import static info.freelibrary.pairtree.MessageCodes.PT_DEBUG_015;
import static info.freelibrary.pairtree.MessageCodes.PT_DEBUG_016;
import static info.freelibrary.pairtree.MessageCodes.PT_DEBUG_029;
import static info.freelibrary.pairtree.MessageCodes.PT_DEBUG_030;
import static info.freelibrary.pairtree.MessageCodes.PT_DEBUG_031;
...
public class FsPairtreeTest extends AbstractFsPairtreeTest {
    ...
    public void testPairtreeDeletion(final TestContext aContext) {
        ...
        myPairtree.create(createResult -> {
            if (createResult.succeeded()) {
                ...
                aContext.assertEquals(true, result, getI18n(PT_DEBUG_031, myPairtree));

                // Then, test ability to delete
                myPairtree.delete(deleteResult -> {
                    if (!deleteResult.succeeded()) {
                        aContext.fail(getI18n(PT_DEBUG_011, myPairtree));
                    } else {
                        if (myFileSystem.existsBlocking(myPairtree.toString())) {
                            aContext.fail(getI18n(PT_DEBUG_015, myPairtree));
                        } else if (myFileSystem.existsBlocking(versionFilePath)) {
                            aContext.fail(getI18n(PT_DEBUG_016, versionFilePath));
                        }
                    }
    ...
    public void testPairtreeCreationConflict(final TestContext aContext) throws IOException {
        ...
        myPairtree.create(result -> {
            if (result.succeeded()) {
                aContext.fail(getI18n(PT_DEBUG_014, myPairtree));
            }
    ...
    public void testPairtreeCreation(final TestContext aContext) {
        ...
        myPairtree.create(result -> {
            if (result.succeeded()) {
                ...
                final String message = getI18n(PT_DEBUG_010, myPairtree);

                aContext.assertEquals(true, exists, message);
            } else {
                aContext.fail(getI18n(PT_DEBUG_029, myPairtree));
            }
    ...
    public void testPairtreeExistsNot(final TestContext aContext) {
        ...
        myPairtree.exists(result -> {
            if (result.succeeded()) {
                if (result.result()) {
                    aContext.fail(getI18n(PT_DEBUG_012, myPairtree));
                }
            } else {
                aContext.fail(getI18n(PT_DEBUG_030, myPairtree));
            }
                ...
                myPairtree.exists(existsResult -> {
                    if (existsResult.succeeded()) {
                        if (!existsResult.result()) {
                            aContext.fail(getI18n(PT_DEBUG_013, myPairtree));
                        }
                    } else {
                        aContext.fail(getI18n(PT_DEBUG_030, myPairtree));
                    }
