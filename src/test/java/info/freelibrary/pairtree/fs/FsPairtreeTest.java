...
public class FsPairtreeTest extends AbstractFsPairtreeTest {
    ...
    public void testNullHandlerExists(final TestContext aContext) {
        try {
            ...
            aContext.fail(getLogger().getMessage(MessageCodes.PT_DEBUG_044));
    ...
    public void testPairtreeDeletion(final TestContext aContext) {
        ...
        myPairtree.create(createResult -> {
            if (createResult.succeeded()) {
                ...
                aContext.assertEquals(true, result, getLogger().getMessage(MessageCodes.PT_DEBUG_031, myPairtree));

                // Then, test ability to delete
                myPairtree.delete(deleteResult -> {
                    if (!deleteResult.succeeded()) {
                        aContext.fail(getLogger().getMessage(MessageCodes.PT_DEBUG_011, myPairtree));
                    } else {
                        if (myFileSystem.existsBlocking(myPairtree.toString())) {
                            aContext.fail(getLogger().getMessage(MessageCodes.PT_DEBUG_015, myPairtree));
                        } else if (myFileSystem.existsBlocking(versionFilePath)) {
                            aContext.fail(getLogger().getMessage(MessageCodes.PT_DEBUG_016, versionFilePath));
                        }
                    }
    ...
    public void testPairtreeCreationConflict(final TestContext aContext) throws IOException {
        ...
        myPairtree.create(result -> {
            if (result.succeeded()) {
                aContext.fail(getLogger().getMessage(MessageCodes.PT_DEBUG_014, myPairtree));
            }
    ...
    public void testPairtreeCreationIfNeeded(final TestContext aContext) {
        ...
        myPairtree.createIfNeeded(result -> {
            if (result.succeeded()) {
                ...
                final String message = getLogger().getMessage(MessageCodes.PT_DEBUG_010, myPairtree);

                aContext.assertEquals(true, exists, message);
            } else {
                aContext.fail(getLogger().getMessage(MessageCodes.PT_DEBUG_029, myPairtree));
            }
    ...
    public void testPairtreeExistsNot(final TestContext aContext) {
        ...
        myPairtree.exists(result -> {
            if (result.succeeded()) {
                if (result.result()) {
                    aContext.fail(getLogger().getMessage(MessageCodes.PT_DEBUG_012, myPairtree));
                }
            } else {
                aContext.fail(getLogger().getMessage(MessageCodes.PT_DEBUG_030, myPairtree));
            }
                ...
                myPairtree.exists(existsResult -> {
                    if (existsResult.succeeded()) {
                        if (!existsResult.result()) {
                            aContext.fail(getLogger().getMessage(MessageCodes.PT_DEBUG_013, myPairtree));
                        }
                    } else {
                        aContext.fail(getLogger().getMessage(MessageCodes.PT_DEBUG_030, myPairtree));
                    }
    ...
    public void testSetPrefix(final TestContext aContext) throws PairtreeException {
        ...
        myPairtree = new PairtreeFactory(myVertx).getPrefixedPairtree(ASDF, new File(myPairtree.getPath()));
        aContext.assertEquals(Optional.of(ASDF), myPairtree.getPrefix());
